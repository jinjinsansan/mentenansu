...
## 🎯 このプロンプトの使用方法
このプロンプトを新しいBoltチャットで使用することで、「かんじょうにっき」プロジェクトを完全に復元できます。
...
### データベースマイグレーション
`supabase/migrations/20250621151756_delicate_limit.sql`と`supabase/migrations/20250624002906_black_palace.sql`ファイルが存在し、すべてのテーブル作成とRLS設定が含まれています。
...
```
src/
├── App.tsx                         # 自動同期フック追加、デバイス認証対応
├── lib/supabase.ts                 # 同意履歴サービス追加、本番環境対応
├── hooks/useSupabase.ts            # 自動同期対応、セキュリティログ対応
├── components/
...
└── pages/
    ├── DiaryPage.tsx               # デバイス認証対応
    ├── DiarySearchPage.tsx         # デバイス認証対応
    └── PrivacyPolicy.tsx           # デバイス認証情報追加
```
...
### 3. デバイス認証システム
- デバイスフィンガープリントによる端末識別
- PIN番号による認証（6桁）
- 秘密の質問による復旧機能
- アカウントロック機能（5回失敗で24時間ロック）
- セキュリティイベントログ
- 管理画面の「デバイス認証」と「セキュリティ」タブで管理
...
このプロンプトを使用することで、プロジェクトを完全な状態で復元し、すべての機能が正常に動作する状態にできます。